...
accession_path="mocks_composition"
...
create_multifasta_script="generate_multifasta_genomes_from_accessions.sh"

# Command to run the InSilicoSeq synthetic metagenome generator
iss_command="iss generate --cpus 10 --genomes ${multigenome_file}"
...
for tsv_file in $(find "$accession_path" -type f -name "*.tsv"); do
  ...
  for model in "${!models[@]}"; do
    ...
    echo "$iss_command --abundance_file $abundance_file $model -o $mock_filename"
    $iss_command --abundance_file $abundance_file $model -o $mock_filename
